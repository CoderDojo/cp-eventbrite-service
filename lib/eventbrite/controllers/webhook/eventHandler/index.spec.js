...
lab.experiment('webhook/eventHandler', () => {
  ...
  lab.test('should create a draft event', (done) => {
    // ARRANGE
    ...
    senecaStub.act
      .withArgs({ role: 'cd-events', cmd: 'listEvents', query: { eventbriteId: 2000, dojoId: 1000 } })
      .callsFake((args, cb) => {
        cb(null, []);
      });
    senecaStub.act
  ...
  lab.experiment('addressToString', () => {
    ...
    lab.test('should concatenate fields in order', (done) => {
      // ARRANGE
      const createdAt = new Date();
      const eventInfoMock = {
        eventbriteId: 2000,
  ...
  lab.test('should update an event', (done) => {
    // ARRANGE
    const createdAt = new Date();
    const eventInfoMock = {
      id: 3000,
    ...
    senecaStub.act
      .withArgs({ role: 'cd-events', cmd: 'listEvents', query: { eventbriteId: 2000, dojoId: 1000 } })
      .callsFake((args, cb) => {
        cb(null, [{ id: 3000, name: 'oldEventTest', description: 'Bring packed lunch ;)' }]);
      });
    senecaStub.act
      .withArgs({
        role: 'cd-events',
